...
public class PassbookController extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        Customer customer = (Customer) request.getSession().getAttribute("customerSession"); // Assuming userId is stored in the session
        
        int customerId = customer.getCustomerId();
        writer.write(customerId);
        List<Transaction> transactions = BankDatabase.getInstance().getTransactionsByUserId(customerId);
